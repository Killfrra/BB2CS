        ...
        public override void SelfExecute()
        {
            ...
            foreach(AttackableUnit unit in GetUnitsInArea((ObjAIBase)owner, owner.Position, 150, SpellDataFlags.AffectEnemies | SpellDataFlags.AffectNeutral | SpellDataFlags.AffectMinions | SpellDataFlags.AffectHeroes, default, true))
            {
                if(GetBuffCountFromCaster(unit, owner, nameof(Buffs.ShenShadowDashCooldown)) == 0)
                {
                    ...
                    SpellEffectCreate(out targetParticle, out _, "shen_shadowDash_unit_impact.troy", default, teamID ?? TeamId.TEAM_UNKNOWN, 10, 0, TeamId.TEAM_UNKNOWN, default, owner, false, owner, default, default, target, default, default, true, default, default, false, false);
                    if(unit is Champion)
                    {
                        if(energyRefunds >= 1)
                        {
...
namespace Buffs
{
    public class ShenShadowDash : BBBuffScript
    {
                    ...
                    if(target is not BaseTurret)
                    {
                        if(GetBuffCountFromCaster(target, default, nameof(Buffs.SharedWardBuff)) == 0)
                        {
                            if(GetBuffCountFromCaster(target, owner, nameof(Buffs.ShenShadowDashCooldown)) == 0)
                            {
                                ...
                                bool nextBuffVars_playParticle; // UNUSED
                                teamID = GetTeamID(owner);
        ...
        public override void OnActivate()
        {
            ...
            SpellEffectCreate(out this.selfParticle, out _, "Shen_shadowdash_mis.troy", default, teamID ?? TeamId.TEAM_UNKNOWN, 10, 0, TeamId.TEAM_UNKNOWN, default, owner, false, owner, default, default, owner, default, default, true, default, default, false, false);
            StartTrackingCollisions(owner, true);
        ...
        public override void OnMoveEnd()
        {
            ...
            foreach(AttackableUnit unit in GetUnitsInArea((ObjAIBase)owner, owner.Position, 200, SpellDataFlags.AffectEnemies | SpellDataFlags.AffectNeutral | SpellDataFlags.AffectMinions | SpellDataFlags.AffectHeroes, default, true))
            {
                if(GetBuffCountFromCaster(unit, owner, nameof(Buffs.ShenShadowDashCooldown)) == 0)
                {
                    ...
                    SpellEffectCreate(out targetParticle, out _, "shen_shadowDash_unit_impact.troy", default, teamID ?? TeamId.TEAM_UNKNOWN, 10, 0, TeamId.TEAM_UNKNOWN, default, owner, false, unit, default, default, unit, default, default, false, default, default, false, false);
                    if(unit is Champion)
                    {
                        if(this.energyRefunds >= 1)
                        {
