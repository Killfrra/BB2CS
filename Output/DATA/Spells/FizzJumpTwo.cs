    ...
    public class FizzJumpTwo : BBBuffScript
    {
        ...
        public override void OnMoveSuccess()
        {
            ...
            if(GetBuffCountFromCaster(owner, owner, nameof(Buffs.FizzJumpBuffered)) == 0)
            {
                level = GetSlotSpellLevel((ObjAIBase)owner, 2, SpellbookType.SPELLBOOK_CHAMPION, SpellSlotType.SpellSlots);
                teamID = GetTeamID(owner);
                SpellEffectCreate(out asdf, out _, "Fizz_TrickSlamTwo.troy", default, teamID, 10, 0, TeamId.TEAM_UNKNOWN, default, owner, false, owner, default, default, owner, default, default, true, false, false, false, false);
                SetTargetable(owner, true);
